...
export default function WoodPricingForm({ clientId }) {
  ...
  const onSubmit = values => {
    ...
    toast.success({
      title: "Part Status",
      message: `${values.wood.length-errors}/${values.wood.length} billing parts successfully updated.`,
    });
  };
        ...
        renderItem={(item, index) => {
          return (
            <View className={"flex-row z-0"} key={index}>
              ...
              <TableInput
                control={control}
                field={`wood[${item.index}].totalCost`}
                containerStyle={"w-1/4 my-0"}
